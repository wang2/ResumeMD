...
function parseHeader(headerLines) {
  ...
  for (let line of headerLines) {
    line = stripMarkdownLinks(line.trim());
    if (line.startsWith('# ')) {
  ...
  if (!name) {
    throw new Error('Resume must start with "# Your Name" header');
  }
  ...
  const renderer = {
    ...
    listitem(token) {
      ...
      if (innerText.includes('</strong> — ') || innerText.includes('</strong> - ')) {
        ...
        if (match) {
          const parts = [
            `    <div class="education-school">${match[1]}</div>`,
            `    <div class="education-degree">${match[2]}</div>`,
            `    <div class="education-date">${match[3]}</div>`,
          ];
